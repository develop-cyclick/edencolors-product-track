...
interface LineItem {
  ...
  expDate: string
  remarks: string
  discrepancyReason: string  // Required when totalQty > actual received (Reconcile)
...
export default function NewGRNPage() {
  ...
  // Compute Reconcile (discrepancy) qty for a line — 0 means no discrepancy
  const getLineDiscrepancy = (l: LineItem): number => {
    if (isReceiveMoreMode) return 0
    const actual = l.usePreGen ? l.scannedItems.length : l.quantity
    if (actual <= 0 || actual >= l.totalQty) return 0
    return l.totalQty - actual
  }
        ...
        .then(async (res) => {
          if (res.success && res.data?.grn) {
            ...
            if (grn.lines && grn.lines.length > 0) {
              ...
              // Enrich with totalQty + discrepancyReason from planLines (if available)
              interface GrnPlanLineItem {
                productMasterId: number
                totalQty: number
                lot: string | null
                expDate: string | null
              ...
              if (grn.planLines && Array.isArray(grn.planLines)) {
                for (const grouped of Object.values(groupedLines)) {
                  const pl = (grn.planLines as GrnPlanLineItem[]).find((p) =>
                    p.productMasterId === grouped.productMasterId &&
                    (p.lot || '') === (grouped.lot || '') &&
                    (p.expDate ? p.expDate.split('T')[0] : '') === (grouped.expDate || '')
                  )
                  if (pl) {
                    grouped.totalQty = pl.totalQty
                    grouped.discrepancyReason = pl.discrepancyReason || ''
                  } else {
                    // Fallback: totalQty matches received qty (no Reconcile)
                    grouped.totalQty = grouped.quantity
                  }
                }
              }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditMode && editingGrnId) {
      ...
      // Validate discrepancy reason when there's a Reconcile balance
      const editMissingReason = lines.filter((l) => getLineDiscrepancy(l) > 0 && !l.discrepancyReason.trim())
      if (editMissingReason.length > 0) {
        await alert({
          title: locale === 'th' ? 'กรุณากรอกเหตุผลส่วนต่าง' : 'Discrepancy Reason Required',
          message: locale === 'th'
            ? `มี ${editMissingReason.length} รายการที่มียอด Reconcile แต่ยังไม่ได้กรอกเหตุผลส่วนต่าง`
            : `${editMissingReason.length} line(s) have a Reconcile balance but no discrepancy reason`,
          variant: 'warning',
          icon: 'warning',
        })
        return
        ...
        const res = await fetch(`/api/warehouse/grn/${editingGrnId}`, {
          ...
          body: JSON.stringify({
            ...
            lines: lines.map((l) => ({
              ...
              expDate: l.expDate || null,
              remarks: l.remarks || null,
              discrepancyReason: getLineDiscrepancy(l) > 0 ? l.discrepancyReason : null,
              preGeneratedItemIds: l.usePreGen ? l.preGeneratedItemIds : undefined,
            })),
          }),
        })
    ...
    // Validate discrepancy reason is required when there's a Reconcile balance
    const missingReason = lines.filter((l) => getLineDiscrepancy(l) > 0 && !l.discrepancyReason.trim())
    if (missingReason.length > 0) {
      await alert({
        title: locale === 'th' ? 'กรุณากรอกเหตุผลส่วนต่าง' : 'Discrepancy Reason Required',
        message: locale === 'th'
          ? `มี ${missingReason.length} รายการที่มียอด Reconcile แต่ยังไม่ได้กรอกเหตุผลส่วนต่าง`
          : `${missingReason.length} line(s) have a Reconcile balance but no discrepancy reason`,
        variant: 'warning',
        icon: 'warning',
      })
      return
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {(() => {
          const reconcileLines = lines
            .map((l) => ({ line: l, diff: getLineDiscrepancy(l) }))
            .filter((x) => x.diff > 0)
          if (reconcileLines.length === 0) return null
          const totalDiff = reconcileLines.reduce((s, x) => s + x.diff, 0)
          return (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-amber-800 mt-1">
                    {locale === 'th'
                      ? `Supplier: ${supplierName || '-'}${poNo ? ` • PO: ${poNo}` : ''} — โปรดติดตามสินค้าที่ขาดจาก Supplier`
                      : `Supplier: ${supplierName || '-'}${poNo ? ` • PO: ${poNo}` : ''} — please follow up with the supplier`}
                  </p>
                  <ul className="mt-3 space-y-1">
                    {reconcileLines.map(({ line: l, diff }) => {
                      const actual = l.usePreGen ? l.scannedItems.length : l.quantity
                      const name = l.productMaster?.nameTh || l.productMaster?.sku || '-'
                      return (
                        <li key={l.id} className="text-xs text-amber-900 flex justify-between gap-3">
                          <span className="truncate">{name}</span>
                          <span className="font-mono whitespace-nowrap">
                            {actual}/{l.totalQty} ({locale === 'th' ? 'ขาด' : 'short'} {diff})
                          </span>
                        </li>
                      )
                    })}
            ...
            {lines.map((line, index) => (
              <div key={line.id} className="border border-[var(--color-beige)] rounded-xl p-5 bg-[var(--color-off-white)]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    {line.usePreGen && !isReceiveMoreMode && (
                      <div className="mb-3">
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <label className="block text-xs font-medium text-[var(--color-charcoal)] mb-1">
                              {locale === 'th' ? 'จำนวนรวมทั้งหมดที่รับเข้า' : 'Total Qty'} <span className="text-red-500">*</span>
                            </label>
                            <input
                              type="number"
                              min={1}
                        ...
                        {(() => {
                          const diff = getLineDiscrepancy(line)
                          if (diff === 0) return null
                          return (
                            <div className="mt-2 p-2.5 rounded-lg bg-amber-50 border border-amber-300 flex items-start gap-2">
                              ...
                              <div className="text-xs">
                                ...
                                <div className="text-amber-700">
                                  {locale === 'th'
                                    ? `ขาดอีก ${diff} ชิ้น (รับจริง ${line.scannedItems.length}/${line.totalQty})`
                                    : `Short by ${diff} unit(s) (received ${line.scannedItems.length}/${line.totalQty})`}
                                </div>
                              </div>
                            </div>
                          )
                              ...
                              return (
                                <div className="mt-2 p-2.5 rounded-lg bg-amber-50 border border-amber-300 flex items-start gap-2">
                                  ...
                                  <div className="text-xs">
                                    ...
                                    <div className="text-amber-700">
                                      {locale === 'th'
                                        ? `ขาดอีก ${diff} ชิ้น (รับจริง ${line.quantity}/${line.totalQty})`
                                        : `Short by ${diff} unit(s) (received ${line.quantity}/${line.totalQty})`}
                                    </div>
                                  </div>
                                </div>
                              )
                  ...
                  {getLineDiscrepancy(line) > 0 && (
                    <div className="lg:col-span-4">
                      <label className="block text-xs font-semibold text-amber-800 mb-1">
                        {locale === 'th' ? 'เหตุผลส่วนต่าง / Discrepancy Reason' : 'Discrepancy Reason'}
                        <span className="text-red-500 ml-1">*</span>
                      </label>
                      <input
                        type="text"
                        value={line.discrepancyReason}
                        onChange={(e) => updateLine(line.id, 'discrepancyReason', e.target.value)}
                        placeholder={locale === 'th'
                          ? 'เช่น Supplier ส่งไม่ครบ รอจัดส่งรอบหน้า'
                          : 'e.g. Supplier shorted, awaiting next delivery'}
                        className="w-full px-3 py-2 text-sm bg-amber-50/50 border border-amber-300 rounded-lg focus:outline-none focus:border-amber-500 focus:shadow-[0_0_0_3px_rgba(217,119,6,0.15)] transition-all placeholder:text-amber-400"
                      />
                    </div>
                  )}
                </div>
