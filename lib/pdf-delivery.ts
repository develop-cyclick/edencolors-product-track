...
export async function generateDeliveryNotePDF(data: DeliveryNoteData): Promise<Buffer> {
  ...
  const instructions = [
    '1. ในกรณีมีแมสฯ ไปส่ง Grab Bike / Lalamove ให้ถ่ายรูปสินค้าที่ได้รับทั้งหมด พร้อมกับแมสฯ ที่ไปส่งว่าได้สินค้าครบหรือไม่',
    '2. กรณีได้รับสินค้าเป็นพัสดุ Kerry / EMS จะต้องบันทึก VDO ขณะที่ทำการเปิดกล่อง และเช็คจำนวนสินค้าว่ามีครบตาม',
    '  ที่ใบส่งสินค้าชั่วคราวระบุไว้หรือไม่',
    '3. กรณีส่งขนส่ง Inter Express กรุณาตรวจสอบบรรจุภัณฑ์ให้เรียบร้อย และเช็คจำนวนสินค้าว่าจำนวนครบตามใบส่งสินค้าชั่วคราวที่ระบุไว้หรือไม่',
  ]
